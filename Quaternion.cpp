...
Quaternion MyMath::SetQuaternion(const Vector3D& v, float angle)
{
    Vector3D vec = v;
    vec.normalize();
...
Quaternion MyMath::Multiply(const Quaternion& q, const Quaternion& r)
{
    Quaternion ans;
    ans.w = q.w * r.w - q.x * r.x - q.y * r.y - q.z * r.z;
    ans.x = q.y * r.z - q.z * r.y + r.w * q.x + q.w * r.x;
...
Quaternion MyMath::Inverse(const Quaternion& quaternion)
{
    Quaternion ans = quaternion;
    float norm = Norm(ans);
    ans /= norm;
...
Quaternion MyMath::MakeAxisAngle(const Vector3D& axis, float angle)
{
    Quaternion ans;
    Vector3D axis_ = axis;
    axis_.normalize();
...
Vector3D MyMath::RotateVector(const Vector3D& vec, const Quaternion& quaternion)
{
    Quaternion vecQ;
    vecQ.SetVec(vec);
...
Matrix MyMath::MakeRotateMatrix(const Quaternion& quaternion)
{
    Matrix mat;
...
Quaternion MyMath::DirectionToDirection(const Vector3D& u, const Vector3D& v)
{
    Vector3D vecU = u;
